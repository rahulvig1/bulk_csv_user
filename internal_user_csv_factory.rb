...
CSV.open(csv_filename, "wb") do |csv|
  csv << ["Email *", "First Name *", "Last Name *", "Title *", "Company *", "Phone *", "Secondary Email id (add email ids with || separation)", "Company Role * (Attendee | Requestor | Executive Admin | Meeting Manager)", "Verification email already sent before? (view only field, no action)", "Send verification email (Resend if already sent before. If left blank, defaults to N.)"]
